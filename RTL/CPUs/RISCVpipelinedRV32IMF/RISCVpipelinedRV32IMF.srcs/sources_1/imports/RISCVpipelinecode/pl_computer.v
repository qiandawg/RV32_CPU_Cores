...
module pl_computer (      // pipelined cpu
    ...
    pl_stage_id id_stage (.mrd(mrd),.mm2reg(mm2reg),.mwreg(mwreg),.erd(erd),.em2reg(em2reg),.ewreg(ewreg),.ecancel(ecancel),.mdwait(mdwait),.dpc(dpc),.inst(inst),
                          ...
                          .fwdla(fwdla),.fwdlb(fwdlb),.fwdfa(fwdfa),.fwdfb(fwdfb),.fwdfe(fwdfe),.e3d(e3d),.dfb(dfb),.ed(ed),.efwdfe(efwdfe),.edata(edata),.jal(jal));                // ID stage
    ...
    pl_reg_de de_reg ( cancel, wreg, m2reg, wmem, call, rv32m,
        aluc, func3, dpc4, da, db, dd, rs1, rs2, rd, fuse, start_sdivide,start_udivide,clk,clrn,
                      ...
                      ealuc,efunc3, epc4, ea,eb, ers1, ers2, erd,estart_sdivide,estart_udivide,wremw,wfpr, ewfpr,ejal,jal,efwdfe,ed,fwdfe);
