...
//    wire i_swc1 = ~ecancel & (opcode == 7'b0100111) & (func3 == 3'b010); // store word from FPR  //  CUSTOM instruction to load to floating point Register   
    wire i_flw = ~ecancel & (opcode == 7'b0000111) & (func3 == 3'b010); // load word to FPR     //  CUSTOM instruction to load to floating point Register TODO change to FLW
    wire i_fls = ~ecancel & (opcode == 7'b0100111) & (func3 == 3'b010); // store word from FPR  //  CUSTOM instruction to load to floating point Register
    ...
    // Control logic
    assign aluc[0] = i_sub | i_xori | i_xor | i_andi | i_add | i_slli | i_srli | i_srai;
    ...
    assign bimm = i_sw | i_lw | i_addi | i_lui | i_slli | i_srli | i_srai | i_xori | i_ori | i_andi;
    ...
    assign wreg = (i_lui | i_jal | i_jalr | i_lw | i_addi | i_xori |
                   ...
                   i_mul | i_mulh | i_mulhsu | i_mulhu | i_div | i_divu | i_rem | i_remu | i_fadd | i_fsub | i_fmul | i_fdiv ) & wpcir;
    ...
    // detect if its floating point instruction, 
    wire       i_fs = i_fadd | i_fsub | i_fmul | i_fdiv | i_fsqrt; // Floating Point Source register
    wire       i_ft = i_fadd | i_fsub | i_fmul | i_fdiv;           // Floating point target register  
    
    assign stall_lw = ewreg & em2reg & (ern != 0) & (i_rs1 & (ern == rs1) |
                                                     i_rs2 & (ern == rs2));
    ...
    assign wfpr     = i_flw & wpcir;             // fp rf y write enable
    ...
    assign swfp       = i_fls;                   // select signal
